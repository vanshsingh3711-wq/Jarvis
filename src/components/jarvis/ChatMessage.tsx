      ...
      <div className="shrink-0 pt-0.5">
        {isUser ? (
          <div className="relative flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-b from-zinc-800 to-zinc-900 border border-white/5 shadow-inner">
            <User size={18} className="text-zinc-400" strokeWidth={1.5} />
          </div>
        ) : (
          ...
          <div className="mt-4 flex flex-col gap-2.5">
            ...
            <div className="flex flex-wrap gap-2">
              {message.sources.map((source) => (
                ...
                >
                  <FileText size={12} className="text-amber-500/70 group-hover/source:text-amber-400 transition-colors" strokeWidth={2} />
                  <span className="truncate max-w-[200px] font-light">{source.title}</span>
                </a>
              ))}
            </div>
          </div>
